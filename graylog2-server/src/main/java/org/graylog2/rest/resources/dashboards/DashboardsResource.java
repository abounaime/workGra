...
public class DashboardsResource extends RestResource {
    ...
    public Response create(@ApiParam(name = "JSON body", required = true) CreateDashboardRequest cr) throws ValidationException {
        // Create dashboard.
        final Dashboard dashboard = dashboardService.create(cr.title(), cr.description(), getCurrentUser().getName(), Tools.nowUTC(),false
        );
    ...
    public void update(@ApiParam(name = "dashboardId", required = true)
        ...
        final Boolean favoriteDash = cr.favoriteDash();
        if(favoriteDash != null) {
            dashboard.setFavoriteDash(cr.favoriteDash());
        }
    System.out.println("UPDAAAAAAAAAAAAAAAATA ::::"+dashboard.isFavoriteDash());
